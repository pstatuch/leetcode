function minDifference(nums: number[]): number {
    ...
    let get4SmallestValues = function(nums: number[]){
    ...
    let get4BiggestValues = function(nums: number[]){
        let result = nums.slice(0, nums.length>=4? 4 : nums.length);
    ...
    function findLargestIndex(nums: number[]){
        let result = 0;
        let largest = nums[0];
    ...
    function findSmallestIndex(nums: number[]){
        let result = 0;
        let smallest = nums[0];
        for(let i =1 ; i<nums.length; i++){
            if(nums[i] < smallest){
